...
// --- TUNING (⚠️ UNCHANGED — YOUR TESTED VALUES) ---
...
// ===================== NEW: IRREGULARITY VARIABLES =====================
#define IRREG_WINDOW 20                 // ~300–400 ms
float tiltBuffer[IRREG_WINDOW];
int tiltIndex = 0;
bool tiltBufferFilled = false;

float tiltIrregularity = 0;             // RMS variability
#define IRREGULARITY_LIMIT 3.0f          // degrees RMS (safe, conservative)
// ======================================================================
...
void loop() {
  ...
  if (mpu.dmpGetCurrentFIFOPacket(FIFOBuffer)) { 
    ...
    float tiltDelta =
      max(fabs(roll - rollBaseline), fabs(pitch - pitchBaseline));
    ...
    // ===================== NEW: IRREGULARITY COMPUTATION =====================
    tiltBuffer[tiltIndex] = tiltDelta;
    tiltIndex++;

    if (tiltIndex >= IRREG_WINDOW) {
      tiltIndex = 0;
      tiltBufferFilled = true;
    }
    ...
    if (tiltBufferFilled) {
      ...
      float var = 0;
      for(int i = 0; i < IRREG_WINDOW; i++)
        var += (tiltBuffer[i] - mean) * (tiltBuffer[i] - mean);
      var /= IRREG_WINDOW;

      tiltIrregularity = sqrt(var);
    }
    // =======================================================================

    // --- SWAY LOGIC (UNCHANGED, JUST AUGMENTED) ---
    
    bool normalSway =
      (tiltDelta > MIN_TILT_TRIGGER) &&
      (gyroEnergy > MIN_GYRO_TRIGGER) &&
      (tiltIrregularity > IRREGULARITY_LIMIT);   // ✅ ADDED

    bool hardTrigger =
      (tiltDelta > HARD_TILT_LIMIT) ||
    ...
    // --- PLOTTER (ADDED IRREGULARITY VISIBILITY) ---
    Serial.print("Gyro:"); Serial.print(gyroEnergy);
    Serial.print(" Tilt:"); Serial.print(tiltDelta);
    Serial.print(" Irreg:"); Serial.print(tiltIrregularity);
    ...
    if (millis() - lastDraw > 100) {
      ...
      display.print("T:"); display.print((int)tiltDelta);

      display.setCursor(0, 12);
      display.print("I:"); display.print(tiltIrregularity, 1);
