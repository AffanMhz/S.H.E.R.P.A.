...
// --- STATE (Volatile copies for thread-safe display) ---
float gyroEnergy = 0;
float rollBaseline = 0;
unsigned long swayStart = 0;
bool swayDetected = false;

volatile float display_gyroEnergy = 0;
volatile float display_rollDelta = 0;
volatile float display_rollIrregularity = 0;
volatile bool display_swayDetected = false;
...
void setup() {
  ...
  // -------- BASELINE CALIBRATION (Pre-RTOS blocking code) --------
  ...
  // No mutex needed here as RTOS tasks haven't launched yet
...
void SensorTask(void *pvParameters) {
  for (;;) {
    ...
    // Lock I2C to strictly read the FIFO
    ...
    if (packetAvailable) { 
      // The dmpGet... math functions parse the buffer locally; no I2C lock needed here
      mpu.dmpGetQuaternion(&q, FIFOBuffer);
      mpu.dmpGetGravity(&gravity, &q);
      mpu.dmpGetYawPitchRoll(ypr, &q, &gravity);
      ...
      // Update globals for display thread
      display_gyroEnergy = gyroEnergy;
      display_rollDelta = rollDelta;
      display_rollIrregularity = rollIrregularity;
      display_swayDetected = swayDetected;
...
void DisplayTask(void *pvParameters) {
  ...
  for (;;) {
    // Thread-safe copy of variables to prevent tearing during draw
    float d_gyro = display_gyroEnergy;
    float d_roll = display_rollDelta;
    float d_irreg = display_rollIrregularity;
    bool d_sway = display_swayDetected;
    ...
    if (xSemaphoreTake(i2cMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
      ...
      display.setCursor(0, 0);
      display.print("G:"); display.print((int)d_gyro);
      display.setCursor(64, 0);
      display.print("R:"); display.print((int)d_roll);

      display.setCursor(0, 12);
      display.print("I:"); display.print(d_irreg, 1);
      
      display.setCursor(0, 30);
      display.setTextSize(2);
      if(d_sway) display.print("SWAY!");
      else display.print("STABLE");
      
      display.display();
      display.setTextSize(1); // Reset for next loop
      
      xSemaphoreGive(i2cMutex);
    }

    // Precise 100ms interval
